...
/**
 * Calculate and return the first host IP address and IPv4 Mapped IPv6 address from a CIDR subnet.
 * @param {string} cidrStr - The IPv4 subnet expressed
 *                 in CIDR format.
 * @param {callback} callback - A callback function.
 * @return {string} (firstIpAddress) - An IPv4 address.
 * @return {string} (ipv4MappedIpv6Add) - An IPv4 Mapped IPv6 address.
 */
function getFirstIpAddress(cidrStr, callback) {
  ...
  if(firstIpAddress!=null)
     ipv4MappedIpv6Add = getIpv4MappedIpv6Address(firstIpAddress);

  return callback(firstIpAddress,ipv4MappedIpv6Add, callbackError);
...
function main() {
  ...
  for (let i = 0; i < sampleCidrsLen; i++) {
    ...
    getFirstIpAddress(sampleCidrs[i], (ipv4, ipv6, error) => {
